...
function renderGraph(focusId) {{
  ...
  node.append('circle')
    ...
    .on('mouseenter', function(e,d) {{ d3.select(this).transition().duration(150).attr('r', r(d)+5); }})
    .on('mouseleave', function(e,d) {{ d3.select(this).transition().duration(150).attr('r', r(d)); }});
