...
s=d3.forceSimulation(gn).force("link",d3.forceLink(ge).id(d=>d.id).distance(d=>{{var ds={dfs(link_dists)};return ds[d.type]||160}}).strength(0.4)).force("charge",d3.forceManyBody().strength({charge}).distanceMax(600)).force("center",d3.forceCenter(W/2,H/2)).force("collision",d3.forceCollide().radius(d=>nr(d)+{collide})).alphaDecay(0.05).on("tick",()=>{{lk.attr("x1",d=>d.source.x).attr("y1",d=>d.source.y).attr("x2",d=>d.target.x).attr("y2",d=>d.target.y);nd.attr("transform",d=>"translate("+d.x+","+d.y+")")}})}}
function sd(d){{
...
if(nfm[d.id])h+='<div style="margin-bottom:8px"><a href="'+nfm[d.id]+'" target="_blank" style="color:#58a6ff;font-size:12px;text-decoration:none">&#x1F4C4; 打开笔记</a></div>';
...
el.innerHTML=h;}}
svg.on("click",()=>{{g.selectAll(".node").classed("selected",false)}});
