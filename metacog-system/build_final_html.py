...
var nd=g.append("g").selectAll("g").data(gn).join("g").attr("class",d=>"node "+(d.type||"domain")).call(d3.drag().on("start",(e,d)=>{{if(!e.active)s.alphaTarget(0.3).restart();d.fx=d.x;d.fy=d.y}}).on("drag",(e,d)=>{{d.fx=e.x;d.fy=e.y}}).on("end",(e,d)=>{{if(!e.active)s.alphaTarget(0);d.fx=null;d.fy=null}}));
var circles=nd.append("circle").attr("r",nr);
circles.on("mouseenter",function(e,d){{focusNode(d,nd,lk);sd(d);d3.select(this).transition().duration(150).attr("r",nr(d)+5)}});
circles.on("mouseleave",function(e,d){{g.selectAll(".node").classed("linked",false);g.selectAll(".edge").classed("linked",false);d3.select(this).transition().duration(150).attr("r",nr(d))}});
